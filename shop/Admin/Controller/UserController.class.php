    ...
    public function tranMoney()
    {
        ...
        if (is_numeric($uid)) {
            if (IS_GET) {
                ...
                $userTranInfo = D("tranmoney")->where("get_type in (0,9)")->where(['pay_id' => $uid])->order("id desc")->select();

            } else if(IS_POST) {
                $startTime=I("startTime");//起始日期
                $endTime=I("endTime");//终止日期
                $startTime=$startTime? strtotime($startTime) : "";
                $endTime=$endTime ? strtotime($endTime) : "";

                if($startTime&&$endTime){
                    $where="get_time>$startTime and getTime<$endTime";
                }elseif ($startTime){
                    $where="get_time>$startTime";

                }elseif ($endTime){
                    $where="get_time<$endTime";

                }
                $userTranInfo = D("tranmoney")->where("get_type in (0,9) and $where")->where(['pay_id' => $uid])->order("id desc")->select();

            }
            $this->assign("uid",$uid);
            $this->assign("userTranInfo", $userTranInfo);
            $this->display();
        }
    }
    /*
