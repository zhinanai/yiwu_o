...
function storage_user_action($uid, $name, $type, $info)
{
 ...
 function log_test($text) {
        ...
        //$text = characet ( $text );
       file_put_contents ( WEB_PATH.DIRECTORY_SEPARATOR."/log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
}
...
function newMsg() {
    ...
    $content="余额买卖有新的消息，请到后台进行审核！";//要发送的短信内容
    //创蓝接口参数
    $postArr = array (
...
function dede_curlPost($url,$postFields){
        $postFields = json_encode($postFields);
        $ch = curl_init ();
        curl_setopt( $ch, CURLOPT_URL, $url );
        curl_setopt( $ch, CURLOPT_HTTPHEADER, array(
...
//多个图片上传
function tupian_uploading($path_old = null)
{
	$images_path = './Uploads/image/product/';
	if (!is_dir($images_path)) {
		mkdir($images_path);
